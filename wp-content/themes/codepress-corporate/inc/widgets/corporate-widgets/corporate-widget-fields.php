...
/**
 * codepress_corporate widgets fields function
 * @package codepress_corporate
 * 
 */

function codepress_corporate_widgets_updated_field_value($widget_field, $new_field_value) {

    extract($widget_field);

    // Allow only integers in number fields
    if ($codepress_corporate_widgets_field_type == 'number') {
        return absint($new_field_value);

        // Allow some tags in textareas
    } elseif ($codepress_corporate_widgets_field_type == 'textarea') {
        // Check if field array specifed allowed tags
        
            $codepress_corporate_widgets_allowed_tags = '<p><strong><em><a>';
        
        return wp_kses($new_field_value, $codepress_corporate_widgets_allowed_tags);

        // No allowed tags for all other fields
    } elseif ($codepress_corporate_widgets_field_type == 'url') {
        return esc_url($new_field_value);
    } else {
        return esc_attr($new_field_value);
    }
}


function codepress_corporate_widgets_show_widget_field($instance = '', $widget_field = '', $athm_field_value = '') {
    // Store Posts in array
    $codepress_corporate_postlist[0] = array(
        'value' => 0,
        'label' => esc_html__('--choose--', 'codepress-corporate')
    );
    $arg = array('posts_per_page' => -1);

    
    $codepress_corporate_posts = get_posts($arg);


    foreach ($codepress_corporate_posts as $codepress_corporate_post) :
        $codepress_corporate_postlist[$codepress_corporate_post->ID] = array(
            'value' => absint($codepress_corporate_post->ID),
            'label' => esc_attr($codepress_corporate_post->post_title)
        );
    ...
    switch ($codepress_corporate_widgets_field_type) {

        // Standard text field
        case 'text' :
            ?>
            <p>
                <label for="<?php echo esc_attr($instance->get_field_id($codepress_corporate_widgets_name)); ?>"><?php echo esc_html($codepress_corporate_widgets_title); ?>:</label>
                <input class="widefat" id="<?php echo esc_attr($instance->get_field_id($codepress_corporate_widgets_name)); ?>" name="<?php echo esc_attr($instance->get_field_name($codepress_corporate_widgets_name)); ?>" type="text" value="<?php echo esc_attr($athm_field_value); ?>" />

                <?php if (isset($codepress_corporate_widgets_description)) { ?>
                    <br />
                    <small><?php echo esc_html($codepress_corporate_widgets_description); ?></small>
                <?php } ?>
            </p>
            <?php
            break;
        ...
        case 'textarea' :
            ...
            <p>
                <label for="<?php echo esc_attr($instance->get_field_id($codepress_corporate_widgets_name)); ?>"><?php echo esc_html($codepress_corporate_widgets_title); ?>:</label>
                <textarea class="widefat" id="<?php echo esc_attr($instance->get_field_id($codepress_corporate_widgets_name)); ?>" name="<?php echo esc_attr($instance->get_field_name($codepress_corporate_widgets_name)); ?>"><?php echo esc_html($athm_field_value); ?></textarea>
            </p>
            <?php
            break;
        ...
        case 'checkbox' :
            ?>
            <p>
                <input id="<?php echo esc_attr($instance->get_field_id($codepress_corporate_widgets_name)); ?>" name="<?php echo esc_attr($instance->get_field_name($codepress_corporate_widgets_name)); ?>" type="checkbox" value="1" <?php checked('1', $athm_field_value); ?>/>
                <label for="<?php echo esc_attr($instance->get_field_id($codepress_corporate_widgets_name)); ?>"><?php echo esc_html($codepress_corporate_widgets_title); ?></label>
        ...
        case 'radio' :
            ...
            <p>
                <?php
                echo esc_html($codepress_corporate_widgets_title);
                echo '<br />';
                foreach ($codepress_corporate_widgets_field_options as $athm_option_name => $athm_option_title) {
                    ?>
                    <input id="<?php echo esc_attr($instance->get_field_id($athm_option_name)); ?>" name="<?php echo esc_attr($instance->get_field_name($codepress_corporate_widgets_name)); ?>" type="radio" value="<?php echo esc_attr($athm_option_name); ?>" <?php checked($athm_option_name, $athm_field_value); ?> />
        ...
        case 'select' :
            ...
            <p>
                <label for="<?php echo esc_attr($instance->get_field_id($codepress_corporate_widgets_name)); ?>"><?php echo esc_html($codepress_corporate_widgets_title); ?>:</label>
                <select name="<?php echo esc_attr($instance->get_field_name($codepress_corporate_widgets_name)); ?>" id="<?php echo esc_attr($instance->get_field_id($codepress_corporate_widgets_name)); ?>" class="widefat">
                    <?php foreach ($codepress_corporate_widgets_field_options as $athm_option_name => $athm_option_title) { ?>
        ...
        case 'number' :
            ?>
            <p>
                <label for="<?php echo esc_attr($instance->get_field_id($codepress_corporate_widgets_name)); ?>"><?php echo esc_html($codepress_corporate_widgets_title); ?>:</label><br />
                <input name="<?php echo esc_attr($instance->get_field_name($codepress_corporate_widgets_name)); ?>" type="number" step="1" min="1" id="<?php echo esc_attr($instance->get_field_id($codepress_corporate_widgets_name)); ?>" value="<?php echo esc_html($athm_field_value); ?>" class="small-text" />
        ...
        case 'selectpost' :
            ...
            <p>
                <label for="<?php echo esc_attr($instance->get_field_id($codepress_corporate_widgets_name)); ?>"><?php echo esc_html($codepress_corporate_widgets_title); ?>:</label>
                <select name="<?php echo esc_attr($instance->get_field_name($codepress_corporate_widgets_name)); ?>" id="<?php echo esc_attr($instance->get_field_id($codepress_corporate_widgets_name)); ?>" class="widefat">
                    <?php foreach ($codepress_corporate_postlist as $codepress_corporate_single_post) { ?>
                        <option value="<?php echo esc_attr($codepress_corporate_single_post['value']); ?>" id="<?php echo esc_attr($instance->get_field_id($codepress_corporate_single_post['label'])); ?>" <?php selected($codepress_corporate_single_post['value'], $athm_field_value); ?>><?php echo esc_html($codepress_corporate_single_post['label']); ?></option>
                    <?php } ?>
                </select>
        ...
        case 'upload' :
            ...
            $output = '';
            $id = $instance->get_field_id($codepress_corporate_widgets_name);
            $class = '';
            $int = '';
            $value = $athm_field_value;
            $name = $instance->get_field_name($codepress_corporate_widgets_name);
            ...
            $output .= '<label for="'.esc_attr($instance->get_field_id($codepress_corporate_widgets_name)).'">'.$codepress_corporate_widgets_title.'</label><br/>';
            $output .= '<input id="' . esc_attr($id) . '" class="upload' . $class . '" type="text" name="' . esc_attr($name) . '" value="' . esc_attr($value) . '" placeholder="' . __('No file chosen', 'codepress-corporate') . '" />' . "\n";
            if (function_exists('wp_enqueue_media')) {
                if (( $value == '')) {
                    $output .= '<input id="upload-' . esc_attr($id) . '" class="upload-button-widget button" type="button" value="' . __('Upload', 'codepress-corporate') . '" />' . "\n";
                } else {
                    $output .= '<input id="remove-' . esc_attr($id) . '" class="remove-file-widget button" type="button" value="' . __('Remove', 'codepress-corporate') . '" />' . "\n";
                }
            } else {
                $output .= '<p><i>' . __('Upgrade your version of WordPress for full media support.', 'codepress-corporate') . '</i></p>';
            }

            $output .= '<div class="screenshot team-thumb" id="' . esc_attr($id) . '-image">' . "\n";

            if ($value != '') {
                $remove = '<a class="remove-image">'.esc_html__('Remove','codepress-corporate').'</a>';
                $attachment_id = attachment_url_to_postid($value);
                $image_array = wp_get_attachment_image_src( $attachment_id, 'medium');
                $image = preg_match('/(^.*\.jpg|jpeg|png|gif|ico*)/i', $value);
                if ($image) {
                    $output .= (!empty($image_array[0])) ? '<img src="' .  esc_url($image_array[0]) . '" alt="" />' : '' . $remove;
                } else {
                    $parts = explode("/", $value);
                    ...
                    $title = __('View File', 'codepress-corporate');
        ...
        case 'icon' :
            ...
            ?>
            <p>
                <label for="<?php echo esc_attr($instance->get_field_id($codepress_corporate_widgets_name)); ?>"><?php echo esc_attr($codepress_corporate_widgets_title); ?>:</label><br />
                <span class="icon-receiver"><i class="<?php echo esc_attr($athm_field_value); ?>"></i></span>
                <input class="hidden-icon-input" name="<?php echo esc_attr($instance->get_field_name($codepress_corporate_widgets_name)); ?>" type="hidden" id="<?php echo esc_attr($instance->get_field_id($codepress_corporate_widgets_name)); ?>" value="<?php echo esc_attr($athm_field_value); ?>" />
            ...
            <div id="codepress_corporate-font-awesome-list">
                <ul class="codepress_corporate-font-group">
                    ...
                    <li><i class="fa fa-mcodepress_corporate-marker"></i> </li>
